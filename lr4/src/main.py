...
numEpoch = 5  # число эпох обучения
...
model = tf.keras.Sequential([
    keras.layers.Conv2D(numFilter, (sizeFilter, sizeFilter), padding='same', activation='relu',
                        input_shape=(28, 28, 1)),  # свертка
    keras.layers.MaxPooling2D((2, 2), strides=2),
    # замена 2 на 2  на максимальное значение из этих ячеек, шаг 2 по обеим осям
    keras.layers.Conv2D(numFilter2, (sizeFilter2, sizeFilter2), padding='same', activation='relu'),  # свёртка
    keras.layers.MaxPooling2D((2, 2), strides=2),
